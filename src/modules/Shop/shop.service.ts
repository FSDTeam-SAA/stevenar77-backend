  ...
  try {
    ...
    const gelatoPayload = {
      ...
      items: orderData.items.map((item: any) => ({
        ...
        files: item.files?.map((f: { type: any; url: any }) => ({
          type: f.type,
          url: f.url,
        })) || [{ type: 'default', url: '' }],
    ...
    const gelatoRes = await axios.post(GELATO_ORDER_API, gelatoPayload, {
      headers: { 'X-API-KEY': GELATO_API_KEY },
    })
...
export const getQuote = async (orderReferenceId: string) => {
  ...
  try {
    ...
    throw new Error(
      err.response?.data?.message || 'Failed to fetch Gelato quote'
    )
  }
}
...
export const submitDraftOrder = async (orderId: string): Promise<any> => {
  try {
    ...
    console.error(
      'Error submitting draft order:',
      err.response?.data || err.message
    )
    throw new Error(
      err.response?.data?.message || 'Failed to submit draft order'
    )
  }
}
...
export const getGelatoOrderById = async (orderId: string, apiKey: string) => {
  try {
    ...
    throw new Error(
      `Failed to fetch Gelato order ${orderId}: ${
